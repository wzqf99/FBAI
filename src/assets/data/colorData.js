...
/* const newColorData = colorSet.map((item) => {
    RGB = `(${item.RGB[0]},${item.RGB[1]},${item.RGB[2]})`
    CMYK = `(${item.CMYK[0]},${item.CMYK[1]},${item.CMYK[2]},${item.CMYK[3]})`
    return { RGB, CMYK, hex: item.hex, name: item.name, pinyin: item.pinyin }
}) */

// colorSet.forEach((item) => item.RGB = `(${item.RGB.join()})`)



// 按照最后一个字进行颜色归类, 得到一个集合
/* [
    {
        'color': 白,
        'num': 52,
...
/* let colorNameArray = new Set((colorSet.map(item => item.name).map(item => item.slice(-1)))) // charAt(item.length - 1)
colorNameArray = [...colorNameArray] */


/* const newArray = []
for (colorname of colorNameArray) {
    let newItems = colorSet.filter((item) => {
        // return item.name.indexOf(colorname) != -1 
        return item.name.slice(-1) == colorname
    })
    console.log(newItems);   // 通过颜色分类的
    let color = colorname
    let num = newItems.length
    let colornames = newItems.map(item => item.name)
    let hexs = newItems.map(item => item.hex)
    newArray.push({
        color,
        num,
        colornames,
